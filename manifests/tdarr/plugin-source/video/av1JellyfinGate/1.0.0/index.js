    ...
    try {
      ...
      if (!markerState.exists) {
        ...
        if (!matchingRules.size) {
          ...
          const remainingLines = markerState.lines.filter(
            (line) => !matchingRules.has(line)
          );
...
function readMarker(marker) {
  ...
  if (invalid || !rules.length) throw new Error('Gate marker has invalid or missing rules.');
...
function isGateRule(line) {
  ...
  const encodedStem = line.slice(1, -2);
