...
FLOW_PLUGINS = ("av1JellyfinGate", "arrLibraryRescan")


def plugin_is_available(value, plugin_name):
    if isinstance(value, dict):
        return any(
            str(value.get(key, "")) == plugin_name
            for key in ("id", "pluginName", "name")
        ) or any(plugin_is_available(item, plugin_name) for item in value.values())
    if isinstance(value, list):
        return any(plugin_is_available(item, plugin_name) for item in value)
    return False


def wait_for_flow_plugin(plugin_name):
    last_error = None
    for _ in range(60):
        try:
            result = request(
                ...
                {"data": {"string": plugin_name, "pluginType": "Local"}},
            )
            if plugin_is_available(result, plugin_name):
                return
            last_error = f"plugin search returned no {plugin_name} result"
        except ReconcileError as error:
            last_error = str(error)
        time.sleep(10)
    raise ReconcileError(f"Tdarr plugin {plugin_name} was not searchable: {last_error}")


def wait_for_flow_plugins():
    for plugin_name in FLOW_PLUGINS:
        wait_for_flow_plugin(plugin_name)
